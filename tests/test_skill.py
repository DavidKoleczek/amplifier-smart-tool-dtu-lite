...
from dtu_lite.core.skill import CAPABILITIES
from dtu_lite.lib import load_manifest, skill, skill_directory, skill_resources
...
def test_skill_is_a_wrapped_document_naming_every_capability() -> None:
    ...
    assert document.startswith('<skill_content name="dtu-lite">')
    assert document.endswith("</skill_content>")
    assert "# dtu-lite" in document
    assert load_manifest().body in document
    for capability in CAPABILITIES:
        assert f"`{capability.name}` [" in document
        assert f"`dtu-lite {capability.name} --help`" in document
...
def test_repository_line_sits_between_the_header_lines_when_declared(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(skill_module, "repository_url", lambda: "https://example.invalid/dtu-lite")
    header = skill_module.skill().splitlines()

    assert header[1].startswith("Skill directory: ")
    assert header[2] == "Repository: https://example.invalid/dtu-lite"
